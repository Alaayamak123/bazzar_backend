...
def purchase_book(id):
    av_response = requests.get(f'http://localhost:4000/books/{id}/stock/availability')
    if av_response.status_code == 200:
        decrease_response = requests.put(f'http://localhost:4000/books/{id}/count/decrease')
        if decrease_response.status_code == 404:
            return make_response(decrease_response.json(),404)
        book_info = requests.get(f'http://localhost:4000/books/{id}')
        print(book_info)
        book = book_info.json()
        ...
        with open('./logs/order_log.txt', 'a') as log:
